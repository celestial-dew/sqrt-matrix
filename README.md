...
易知 $`\mathbf{A}\mathbf{X}=\mathbf{X}^3=\mathbf{X}\mathbf{A}`$，即 $`\mathbf{X}`$ 保持 $`\mathbf{A}`$ 各特征子空间不变
...
即 $`\mathbf{X}\mathbf{\xi}_i`$ 仍为 $`(\lambda_i\mathbf{E}_{n}-\mathbf{A})\mathbf{\alpha}=\mathbf{0}`$ 的解，即 $`\mathbf{X}\mathbf{\xi}_i`$ 各列均可由 $`\mathbf{\xi}_i`$ 的列向量线性表出
...
且 $`(\sqrt{\lambda_i}\mathbf{E}_{k_i}-\mathbf{Y}_i)(-\sqrt{\lambda_i}\mathbf{E}_{k_i}-\mathbf{Y}_i)=\mathbf{O}`$，即极小多项式无重根
...
记 $`p_i=r(\sqrt{\lambda_i}\mathbf{E}_{k_i}-\mathbf{Y}_i)`$，则 $`r(-\sqrt{\lambda_i}\mathbf{E}_{k_i}-\mathbf{Y}_i)=k_i-p_i`$

则 $`\mathbf{Y}_i`$ 的特征值 $`\sqrt{\lambda_i}`$、$`-\sqrt{\lambda_i}`$ 的线性无关的特征向量个数分别为 $`k_i-p_i`$、$`k_i-(k_i-p_i)=p_i`$

则 $`\mathbf{Y}_i`$ 的线性无关的特征向量个数为 $`k_i-p_i+p_i=k_i`$
...
（2）当 $`\lambda_i=0`$ 时，由 $`\mathbf{A}\mathbf{\xi}_i=\mathbf{O}`$ 得 $`n-r(\mathbf{A})=k_i`$

由 $`r(\mathbf{X})=r(\mathbf{A})`$ 得 $`\mathbf{X}\mathbf{\beta}=\mathbf{0}`$ 有 $`n-r(\mathbf{X})=k_i`$ 个线性无关的解 $`\mathbf{\beta}_1,\mathbf{\beta}_2,...,\mathbf{\beta}_{k_i}`$

则 $`\mathbf{B}=[\mathbf{\beta}_1,\mathbf{\beta}_2,...,\mathbf{\beta}_{k_i}]`$ 和 $`\mathbf{\xi}_i`$ 的列向量为 $`\mathbf{R}^{k_i}`$ 的两个基，即 $`\exists\ k_i`$ 阶可逆矩阵 $`\mathbf{C}`$，使 $`\mathbf{B}=\mathbf{\xi}_i\mathbf{C}`$

则 $`\mathbf{X}\mathbf{B}=\mathbf{X}\mathbf{\xi}_i\mathbf{C}=\mathbf{\xi}_i\mathbf{Y}_i\mathbf{C}=\mathbf{O}`$ 得 $`\mathbf{Y}_i=\mathbf{O}`$ 即 $`\mathbf{D}_i=\mathbf{O}`$，并取 $`\mathbf{R}_i=\mathbf{E}_{k_i}`$
...
- 若 $`\mathbf{A}`$ 对称，$`\mathbf{X}`$ 半正定，则 $`\lambda_{\mathbf{X}}=\lambda_{\mathbf{D}}\ge 0`$ 即 $`\mu_i\ge 0`$ 得 $`p_i=0`$ 即 $`\mathbf{D}_i=\sqrt{\lambda_i}\mathbf{E}_{k_i}`$，则 $`\mathbf{Y}_i=\mathbf{R}_i\mathbf{D}_i\mathbf{R}_i^{-1}=\mathbf{D}_i`$，则 $`\mathbf{X}=\mathbf{P}diag(\mathbf{D}_1,\mathbf{D}_2,...,\mathbf{D}_m)\mathbf{P}^{-1}`$ 唯一，且为主平方根

- 设 $`f(x)`$ 为多项式函数，若 $`\mathbf{X}=f(\mathbf{A})`$，则 $`\mathbf{Y}=\mathbf{P}^{-1}\mathbf{X}\mathbf{P}=\mathbf{P}^{-1}f(\mathbf{A})\mathbf{P}=f(\mathbf{\Lambda})`$，则 $`\mu_i=f(\lambda_i)=f(\mu_i^2)`$ 唯一，即 $`\mathbf{Y}_i`$ 内特征值同号，即 $`p_i\in\{k_i,0\}`$