...
由 $`r(\mathbf{X})=r(\mathbf{A})`$ 得 $`\mathbf{X}\mathbf{\beta}=\mathbf{0}`$ 和 $`\mathbf{A}\mathbf{\beta}=\mathbf{0}`$ 同解（或根据其基础解系等价）
...
- 若 $`k_i\equiv 1`$ 即 $`\mathbf{A}`$ 特征值互异，则 $`p_i\in\{0,1\}`$，由 $`\mathbf{Y}_i^2=[\lambda_i]`$ 得 $`\mathbf{Y}_i=[\pm\sqrt{\lambda_i}]`$，则取 $`\mathbf{D}_i=\mathbf{Y}_i`$，$`\mathbf{R}_i=\mathbf{E}_1`$，则 $`\mathbf{X}=\mathbf{P}diag(\pm\sqrt{\lambda_1},\pm\sqrt{\lambda_2},...,\pm\sqrt{\lambda_n})\mathbf{P}^{-1}`$ 有 $`2n`$ 个解
